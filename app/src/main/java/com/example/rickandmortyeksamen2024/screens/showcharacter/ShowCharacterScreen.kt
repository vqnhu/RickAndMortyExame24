...
fun ShowCharacterScreen(showCharacterViewModel: ShowCharacterViewModel) {
    ...
    val characters by showCharacterViewModel.characters.collectAsState()

    // vise alle karakterene når kjermen lastes
    LaunchedEffect(Unit) {
        showCharacterViewModel.loadAllCharacters()
    }

    Column {
        // vise kun 10 karakterer omgangen
        LazyColumn {
            items(characters.take(10)) { character ->
